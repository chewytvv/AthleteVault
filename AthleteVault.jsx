...
function mediaLimits(plan,tier){if(plan==="free")return{photos:1,videos:1};if(tier==="rookie")return{photos:3,videos:2};if(tier==="rising")return{photos:5,videos:4};return{photos:99,videos:99};}
const BAD_WORDS=["fuck","shit","ass","dick","cock","pussy","bitch","nigga","cunt","damn","hell","sex","xxx","nude","porn"];
function isCleanCode(c){return /^[A-Z0-9]{1,6}$/.test(c)&&!BAD_WORDS.some(w=>c.toLowerCase().includes(w));}
...
function EuroTeams({athlete}){
  ...
  const filtered=GLOBAL_TEAMS.filter(t=>{
    ...
    const mCat=catF==="All"||(t.sportCategory||t.sport)===catF;
    return mS&&mSp&&mC&&mL&&mCat;
  });
  const leagueColor={"GFL1":C.gold,"ELF":C.orange,"EuroLeague":C.purple,"NWSL":C.pink,"Top 14":C.green,"URC":C.teal,"SHL":C.blue,"KHL":C.red,"IPL":C.gold,"BBL":C.green,"Premier League":C.blue,"Bundesliga":C.red,"NFL":C.blue,"CFL":C.red,"XFL":C.orange,"NBA":C.purple,"MLB":C.blue,"NPB":C.red,"KBO":C.orange,"La Liga":C.red,"Serie A":C.blue,"MLS":C.green};
...
function AContent({athlete,saveAthletes,athletes}){
  ...
  function addVideo(){if(videos.length>=vidLim){alert("Video limit reached ("+vidLim+"). Upgrade your plan for more uploads.");return;}if(!vid.title)return;const v={id:Date.now(),...vid,added:new Date().toISOString().slice(0,10)};saveAthletes(prev=>prev.map(a=>String(a.id)===String(athlete.id)?{...a,videos:[v,...(a.videos||[])]}:a));setVid({title:"",platform:[],notes:"",url:""});setShowUpload(false);}
...
function AProfile({athlete,saveAthletes}){
  ...
  const photoLim=mediaLimits(athlete.plan,athlete.tier).photos;
  const profileUrl=`${window.location.origin}${window.location.pathname}#/p/${f.profileCode||athlete.id}`;
  function save(){
    if(f.profileCode&&!isCleanCode(f.profileCode)){setCodeErr("Code must be 1–6 uppercase letters/numbers, no profanity.");return;}
    setCodeErr("");
    saveAthletes(prev=>prev.map(a=>String(a.id)===String(athlete.id)?{...a,...f,followers:Number(f.followers)}:a));
    setSaved(true);setTimeout(()=>setSaved(false),2000);
  }
  function handlePhotoFile(e){
    const file=e.target.files?.[0];if(!file)return;
    if(!athlete.profilePhoto&&athlete.plan==="free"&&f.profilePhoto){alert("Free plan allows 1 profile photo. Upgrade to add more.");return;}
    const reader=new FileReader();
    reader.onload=ev=>setF(p=>({...p,profilePhoto:ev.target.result}));
    reader.readAsDataURL(file);
  }
  const WORK_AUTH_OPTIONS=["USA","UK","EU","Canada","Australia","Japan","South Korea","Brazil","UAE","Saudi Arabia"];
  ...
  return <div>
    ...
    <Card style={{marginTop:13}}>
      <div style={{color:C.muted,fontSize:10,fontFamily:"DM Mono,monospace",letterSpacing:1,marginBottom:13}}>PROFILE PHOTO <span style={{color:C.mutedHi}}>({athlete.plan==="free"?"1 max":photoLim===99?"unlimited":photoLim+" max"})</span></div>
      <div style={{display:"flex",gap:14,alignItems:"flex-start",flexWrap:"wrap"}}>
        {f.profilePhoto&&<img src={f.profilePhoto} style={{width:80,height:80,borderRadius:"50%",objectFit:"cover",border:`2px solid ${C.accent}`}} alt="Profile"/>}
        <div style={{display:"flex",flexDirection:"column",gap:9,flex:1}}>
          <Inp label="PHOTO URL" value={f.profilePhoto} onChange={v=>setF(p=>({...p,profilePhoto:v}))} placeholder="https://example.com/photo.jpg"/>
          <div style={{color:C.muted,fontSize:11}}>— or upload a file —</div>
          {(athlete.plan!=="free"||!athlete.profilePhoto)?<input type="file" accept="image/*" onChange={handlePhotoFile} style={{color:C.white,fontSize:12,cursor:"pointer"}}/>:<div style={{color:C.gold,fontSize:12}}>⭐ Upgrade for more photos</div>}
        </div>
      </div>
    </Card>
    {/* Profile Code */}
    <Card style={{marginTop:13}}>
      <div style={{color:C.muted,fontSize:10,fontFamily:"DM Mono,monospace",letterSpacing:1,marginBottom:8}}>CUSTOM PROFILE CODE</div>
      <div style={{display:"flex",gap:10,alignItems:"flex-end",flexWrap:"wrap"}}>
        <div style={{flex:1}}><Inp label="CODE (1–6 CHARS, A–Z / 0–9)" value={f.profileCode} onChange={v=>setF(p=>({...p,profileCode:v.toUpperCase().replace(/[^A-Z0-9]/g,"").slice(0,6)}))} placeholder="MYCODE"/></div>
      </div>
      {codeErr&&<div style={{color:C.red,fontSize:12,marginTop:6}}>{codeErr}</div>}
      <div style={{color:C.mutedHi,fontSize:11,marginTop:6,fontFamily:"DM Mono,monospace"}}>
        Public URL: athletevault.org/#/p/{f.profileCode||athlete.id}
      </div>
    </Card>
    {/* International */}
    <Card style={{marginTop:13}}>
      <div style={{color:C.muted,fontSize:10,fontFamily:"DM Mono,monospace",letterSpacing:1,marginBottom:13}}>INTERNATIONAL OPPORTUNITIES</div>
      <div style={{display:"flex",flexDirection:"column",gap:11}}>
        <label style={{display:"flex",alignItems:"center",gap:9,cursor:"pointer"}}>
          <input type="checkbox" checked={f.hasPassport} onChange={e=>setF(p=>({...p,hasPassport:e.target.checked}))} style={{width:16,height:16,accentColor:C.accent}}/>
          <span style={{color:C.white,fontSize:13}}>I have a valid passport</span>
        </label>
        <label style={{display:"flex",alignItems:"center",gap:9,cursor:"pointer"}}>
          <input type="checkbox" checked={f.seekingInternational} onChange={e=>setF(p=>({...p,seekingInternational:e.target.checked}))} style={{width:16,height:16,accentColor:C.accent}}/>
          <span style={{color:C.white,fontSize:13}}>Open to international opportunities</span>
        </label>
        <div><div style={{color:C.muted,fontSize:10,fontFamily:"DM Mono,monospace",letterSpacing:1,marginBottom:7}}>WORK AUTHORIZATION</div><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{WORK_AUTH_OPTIONS.map(o=>{const on=f.workAuth.includes(o);return<button key={o} onClick={()=>setF(p=>({...p,workAuth:on?p.workAuth.filter(x=>x!==o):[...p.workAuth,o]}))} style={{background:on?C.accentGlow:"transparent",border:`1px solid ${on?C.accent:C.border}`,borderRadius:7,padding:"4px 10px",cursor:"pointer",color:on?C.accent:C.muted,fontSize:12,fontFamily:"'Sora',sans-serif",fontWeight:600}}>{o}</button>;})}
        </div></div>
        <div><div style={{color:C.muted,fontSize:10,fontFamily:"DM Mono,monospace",letterSpacing:1,marginBottom:7}}>PREFERRED COUNTRIES</div><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{["Germany","France","Spain","Italy","UK","Canada","Australia","Japan","South Korea","Brazil","Turkey","Netherlands","Greece","Austria","Sweden"].map(o=>{const on=f.preferredCountries.includes(o);return<button key={o} onClick={()=>setF(p=>({...p,preferredCountries:on?p.preferredCountries.filter(x=>x!==o):[...p.preferredCountries,o]}))} style={{background:on?C.goldGlow:"transparent",border:`1px solid ${on?C.gold:C.border}`,borderRadius:7,padding:"4px 10px",cursor:"pointer",color:on?C.gold:C.muted,fontSize:12,fontFamily:"'Sora',sans-serif",fontWeight:600}}>{o}</button>;})}
        </div></div>
      </div>
    </Card>
...
function CoachEuroTeams({coach}){
  ...
  const SPORT_CATS2=["All","American Football","Soccer","Basketball","Baseball","Rugby","Ice Hockey","Cricket","Other"];
  const [catF2,setCatF2]=useState("All");
  const countries=[...new Set(GLOBAL_TEAMS.map(t=>t.country))];
  const filtered=GLOBAL_TEAMS.filter(t=>{const mS=(t.name+t.league+(t.city||"")).toLowerCase().includes(search.toLowerCase());const mSp=sportF==="all"||t.sport===sportF;const mC=countryF==="all"||t.country===countryF;const mCat=catF2==="All"||(t.sportCategory||t.sport)===catF2;return mS&&mSp&&mC&&mCat;});
  const leagueColor={"GFL1":C.gold,"ELF":C.orange,"EuroLeague":C.purple,"NWSL":C.pink,"Top 14":C.green,"URC":C.teal,"SHL":C.blue,"KHL":C.red,"IPL":C.gold,"BBL":C.green,"Premier League":C.blue,"Bundesliga":C.red,"NFL":C.blue,"CFL":C.red,"XFL":C.orange,"NBA":C.purple,"MLB":C.blue,"NPB":C.red,"KBO":C.orange,"La Liga":C.red,"Serie A":C.blue,"MLS":C.green};
  ...
  return <div>
    <Sec title="Global Teams" sub={`Find coaching opportunities at ${GLOBAL_TEAMS.length}+ pro teams worldwide`}/>
    <p style={{color:C.mutedHi,fontSize:13,marginBottom:14,lineHeight:1.6}}>As a coach, you can reach out to organizations worldwide for staff positions, player development roles, or to place your athletes.</p>
    <div style={{display:"flex",gap:6,marginBottom:10,flexWrap:"wrap"}}>{SPORT_CATS2.map(c=><button key={c} onClick={()=>setCatF2(c)} style={{background:catF2===c?C.goldGlow:"transparent",border:`1px solid ${catF2===c?C.gold:C.border}`,borderRadius:7,padding:"5px 12px",cursor:"pointer",color:catF2===c?C.gold:C.muted,fontSize:12,fontFamily:"'Sora',sans-serif",fontWeight:600}}>{c}</button>)}</div>
    <div style={{display:"flex",gap:8,marginBottom:14,flexWrap:"wrap"}}>
      <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search teams, leagues, cities…" style={{flex:1,minWidth:160,background:C.card,border:`1px solid ${C.border}`,borderRadius:8,padding:"9px 13px",color:C.white,fontSize:13,outline:"none",fontFamily:"'Sora',sans-serif"}}/>
      <select value={countryF} onChange={e=>setCountryF(e.target.value)} style={{background:C.dark,border:`1px solid ${C.border}`,borderRadius:8,padding:"9px 12px",color:C.white,fontSize:13,outline:"none"}}><option value="all">All Countries</option>{countries.map(c=><option key={c}>{c}</option>)}</select>
    </div>
      ...
      <Card glow color={C.accent} style={{marginBottom:16,overflow:"hidden",padding:0}}>
        ...
        <div style={{padding:"0 24px 24px",marginTop:-44}}>
          <div style={{display:"inline-block",padding:4,borderRadius:"50%",background:C.card,border:`3px solid ${C.accent}`,marginBottom:12,boxShadow:`0 0 24px ${C.accent}44`}}>
            <Avatar name={a.name} size={68} color={C.accent} verified={a.verified} photo={a.profilePhoto||""}/>
          </div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",flexWrap:"wrap",gap:10}}>
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
              ...
              {a.hasPassport&&<Badge color={C.teal}>🛂 Has Passport</Badge>}
              {a.seekingInternational&&<Badge color={C.green}>🌍 Open to International</Badge>}
            </div>
          </div>
        </div>
      </Card>
